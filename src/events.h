...
class Events
{
	struct EventsInfo {
		...
		int32_t playerOnEquip = -1;
		int32_t playerOnDeEquip = -1;
	...
	public:
		...
		bool eventPlayerOnEquip(Player* player, Item* item, slots_t slot, bool isCheck);
		void eventPlayerOnDeEquip(Player* player, Item* item, slots_t slot);
